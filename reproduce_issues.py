...
def test_fan_rotom_turn_3():
    ...
    # Find action index for USE_ACTIVE_ABILITY
    # In action_table logic, USE_ACTIVE_ABILITY is a specific action kind.
    # We need to find the index corresponding to "USE_ACTIVE_ABILITY"
    ...
    if can_use:
        ...
        # Try to execute it just to see what happens
        # We need to satisfy checks in step() as well
        # Note: step() logic for Fan Rotom checks is_first_turn again (lines 579+)
        obs, reward, done, truncated, info = env.step(ability_idx)
        print("Executed step. Check logs for 'Fan Call' print.")
    else:
...
def test_night_stretcher_empty_discard():
    ...
    # Find action for PLAY_TRAINER Night Stretcher
    from tcg.actions import ACTION_TABLE
    ns_idx = -1
    for i, act in enumerate(ACTION_TABLE):
        if act.kind == "PLAY_TRAINER" and act.a == "Night Stretcher":
            # Target 6 (Global) usually
            if act.b == 6:
                ns_idx = i
                break
    
    if ns_idx == -1:
        print("Error: Could not find Night Stretcher action")
        return
